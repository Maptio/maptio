...
export class Auth {
  ...
  private fullstory: any = window['FS']; // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  public loginMaptioApi(email: string, password: string) {
    ...
    EmitterService.get('maptio_api_token').emit('TODO');
    ...
    //   function (err: any, authResult: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  /**
   * Retrieve all user information stored in Auth0
   * @param userId
   * @returns A user object created from the data stored in Auth0
   */
