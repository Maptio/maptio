...
@Component({
    ...
    standalone: true,
    imports: [NgIf, RouterLink, FormsModule, ReactiveFormsModule, PermissionsDirective, ConfirmationPopoverModule]
})
export class MapCardComponent implements OnInit, OnChanges {
  ...
  private alertAboutOutdatedDataset() {
    alert(
      $localize`A friendly heads-up: Your map has been changed by another user (or by you in a different browser tab). Please hit refresh to load the latest version before archiving or restoring your map. Sorry for the hassle.`
    );
  }
}
