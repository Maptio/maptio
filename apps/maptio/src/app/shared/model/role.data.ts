...
export class Role implements Serializable<Role> {
  ...
  convertToLibraryRole(): void {
    if (this.isLibraryRole()) {
      console.error(
        `Attempting to convert a library role (${this.shortid}) to a library role. Aborting.`
      );
      return;
    }
  ...
  convertToCustomRole() {
    if (this.isCustomRole()) {
      console.error(
        `Attempting to convert a custom role (${this.title}) to a custom role. Aborting.`
      );
      return;
    }
