...
describe('jwt.service.ts', () => {
  ...
  it(
    ...
    waitForAsync(
      inject(
        [JwtEncoder, Http, MockBackend],
        (target: JwtEncoder, http: Http, mockBackend: MockBackend) => {
          const mockResponse = { token: 'abcdefghijklmn' };

          mockBackend.connections.subscribe((connection: MockConnection) => {
            if (
              connection.request.method === RequestMethod.Post &&
              connection.request.url === '/api/v1/jwt/encode'
            ) {
          ...
          mockBackend.connections.subscribe((connection: MockConnection) => {
            if (
              ...
              connection.request.url ===
                '/api/v1/jwt/decode/' + 'abcdefghijklmn'
            ) {
              connection.mockRespond(
                new Response(
                  new ResponseOptions({
