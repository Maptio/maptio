...
export class MappingZoomableComponent implements IDataVisualizer {
  ...
  init() {
    ...
    try {
      ...
      innerSvg.call(
        ...
        d3.zoomIdentity.translate(initX, initY).scale(initK)
      );
    ...
    this.resetSubscription = this.isReset$
      ...
      .subscribe((isReset) => {
        ...
        innerSvg
          .transition()
          .duration(this.ZOOMING_TRANSITION_DURATION)
          .call(
            this.zooming.transform,
    ...
    this.zoomSubscription = this.zoom$.subscribe((zf: number) => {
      try {
        // the zoom generates an DOM Excpetion Error 9 for Chrome (not tested on other browsers yet)
        if (zf) {
          this.zooming.scaleBy(
            innerSvg.transition().duration(this.ZOOMING_TRANSITION_DURATION),
            zf
          );
        } else {
          innerSvg
    ...
    highlightInitiative.subscribe((node) => {
      innerSvg
        .select(`circle.node.initiative-map[id="${node.id}"]`)
        .dispatch('click');
    });
    ...
    */
    ...
    this.outerFontScale = d3
      .scaleLog()
      .domain(defaultScaleExtent)
      .range(outerFontSizeRange);
    this.innerFontScale = d3
      .scaleLog()
      .domain(defaultScaleExtent)
      .range(innerFontSizeRange);
  ...
  adjustViewToZoomEvent(g: any, event: any, force?: boolean): void {
    ...
    const scaleExtent: Array<number> = this.zooming.scaleExtent()
    ...
    g.selectAll('circle.node').each((d: any) => (d.zf = zoomFactor));

    g.selectAll('.node.no-children').each(function (d: any): void {
      myInnerFontScale.range([
        (d.r * Math.PI) / MAX_NUMBER_LETTERS_PER_CIRCLE,
        3,
      ]);
      select(this)
    ...
    const getAccountableRadius = (d: any) =>
      d.children ? outerFontSize * 0.75 : myInnerFontScale(zoomFactor) * 1.5;
  ...
  update(
    ...
    initiativeWithChildrenEnter
      .append('text')
      .attr('class', 'name with-children')
      .classed('initiative-map', true);
    initiativeNoChildrenEnter
      .append('foreignObject')
      .attr('class', 'name no-children')
      .classed('initiative-map', true);

    initiativeWithChildrenEnter
      .append('circle')
      .attr('class', 'accountable with-children')
      .classed('initiative-map', true);
    initiativeNoChildrenEnter
      .append('circle')
      .attr('class', 'accountable no-children')
      .classed('initiative-map', true);
    ...
    initiativeNoChildren
      .select('foreignObject.name.no-children')
      .attr('id', function (d: any) {
        return `${d.data.id}`;
      })
      .classed('initiative-map', true)
    ...
    if (localStorage.getItem('node_id')) {
      ...
      if (
        ...
        getLastZoomedCircle().data.id.toString() === id.toString()
      )
        return;
      this.svg
        .select(`circle.node.initiative-map[id="${id}"]`)
        .dispatch('click');
    }

    return Promise.resolve(nodes);

    function getClickedElementCoordinates(
      clickedElement: any,
      newScale: number,
      translateX: number,
      translateY: number
      ...
      if (
        ...
        (clickedElement.transform.baseVal.length > 0 ||
          clickedElement.transform.baseVal.numberOfItems > 0)
      ) {
        clickedX =
          clickedElement.transform.baseVal.getItem(0).matrix.e * newScale;
        clickedY =
          clickedElement.transform.baseVal.getItem(0).matrix.f * newScale;
        clickedX -= margin;
        clickedY -= margin;
      } else {
    ...
    function zoom(focus: any, clickedElement?: any): void {
      ...
      const newScale: number =
        focus === root || focus.parent === root
          ? 1
          : getViewScaleForRadius(focus.r);
      const coordinates: Array<number> = getClickedElementCoordinates(
        clickedElement,
        newScale,
        focus.translateX,
        focus.translateY
      ...
      svg
        ...
        .call(
          zooming.transform,
          d3.zoomIdentity
            .translate(view[0] - coordinates[0], view[1] - coordinates[1])
            .scale(newScale)
        );
    }
    ...
    function initMapElementsAtPosition(v: any) {
      ...
      node
        ...
        .duration((d: any): number =>
          d.children ? TRANSITION_DURATION / 5 : TRANSITION_DURATION / 5
        )
        .attr(
          'transform',
          (d: any): string => `translate(${d.x - v[0]}, ${d.y - v[1]})`
        )
      ...
      circle
        ...
        .style('stroke', function (d: any) {
          return d.children
            ...
            ? d3.color(color(d.depth)).darker(1).toString()
            : null;
        })
        ...
        .style('fill-opacity', function (d: any) {
          return d.children ? 0.1 : !d.children && d.parent === root ? 0.1 : 1;
        })
    ...
    function addCircle(groups: any): void {
      groups
        ...
        .classed(
          'with-border',
          (d: any): boolean => !d.children && d.parent === root
        )
        .on(
          'click',
          function (d: any, index: number, elements: Array<HTMLElement>): void {
    ...
    function exitWithAnimations(groups: any) {
      groups.exit().select('text').remove();
      groups.exit().select('foreignObject').remove();
      groups.exit().select('circle.accountable').remove();
      groups
        .exit()
        .select('circle.node')
      ...
      if (eventName == 'contextmenu') {
        el.on('contextmenu', function (d: any): void {
          ...
          d3.select(`circle.node[id="${d.data.id}"]`).dispatch('contextmenu', {
            bubbles: true,
            cancelable: true,
            detail: { position: [mouse[0], mouse[1]] },
          });
        });
      } else {
        el.on(eventName, function (d: any): void {
