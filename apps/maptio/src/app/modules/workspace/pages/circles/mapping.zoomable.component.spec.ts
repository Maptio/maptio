...
describe('mapping.zoomable.component.ts', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(mockDataService, 'get').and.returnValue(
      observableOf({
        initiative: data,
        dataset: new DataSet({ datasetId: '123' }),
        team: new Team({}),
        members: [],
      })
    );
    ...
    spyOn(localStorage, 'getItem').and.returnValue(
      `{"fontColor":"#000","mapColor":"#aaa","fontSize":1,"explorationMode":false}`
    );
  ...
  xit('should draw SVG centered when data is valid', () => {
    ...
    expect(svg.querySelector('g').getAttribute('transform')).toBe(
      `translate(${component.translateX}, ${component.translateY}) scale(1)`
    );
  });
  ...
  xit('should draw SVG with correct text labels  when data is valid', () => {
    ...
    expect(
      g.querySelectorAll('.name')[1].querySelector('foreignObject')
    ).toBeDefined();
    expect(
      g.querySelectorAll('.name')[2].querySelector('foreignObject')
    ).toBeDefined();
  });
