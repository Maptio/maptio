...
describe('building.component.ts', () => {
              ...
              },
              {
                ...
                useClass: class {
                  ...
                  getUser = jest
                    .fn()
                    .mockReturnValue(
                      observableOf(new User({ user_id: 'UID' }))
                    );
                },
              },
              {
      ...
      waitForAsync(() => {
        ...
        component.loadData(dataset, team, team.members).then(() => {
          ...
          spyDataService.calls
            ...
            .then(() => {
              expect(component.nodes[0].children[0].accountable.picture).toBe(
                'URL1'
              );
              expect(component.nodes[0].children[1].accountable.picture).toBe(
                'URL2'
              );
              expect(
                component.nodes[0].children[2].accountable
              ).toBeUndefined();
              expect(component.nodes[0].children[0].accountable.name).toBe(
                'Name1'
              );
              expect(component.nodes[0].children[1].accountable.name).toBe(
                'Name2'
              );
              expect(
                component.nodes[0].children[2].accountable
  ...
  describe('Remove node', () => {
    it('should remove from root node', () => {
      ...
      (root.id = -1), (node1.id = 1), (node2.id = 2);
      node3.id = 3;
      (node1.name = 'first'), (node2.name = 'second');
      node3.name = 'third';
      root.name = 'root';
  ...
  describe('Add node', () => {
    ...
    it('should add to children nodes in first place', () => {
      ...
      expect(component.nodes[0].children[0].children[0].team_id).toBe(
        'team_id'
      );
