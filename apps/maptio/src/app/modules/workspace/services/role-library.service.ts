...
export class RoleLibraryService {
  ...
  findRoleInLibrary(role: Role): Role {
    if (role.isCustomRole()) {
      console.error(
        'Attempted to search for a custom role with role library service.'
      );
      return;
    }
  ...
  addRoleToLibrary(role: Role): void {
    ...
    if (this.findRoleInLibrary(role)) {
      console.error(
        'Attempted to add a role to the role library when it is already there.'
      );
      return;
    }
  ...
  editRole(role: Role): void {
    if (role.isCustomRole()) {
      console.error(
        'Attempted to save edits to a custom role with role library service.'
      );
      return;
    }
  ...
  deleteRoleFromLibrary(role: Role): void {
    if (role.isCustomRole()) {
      console.error(
        'Attempted delete a custom role with role library service.'
      );
      return;
    }
    ...
    if (!libraryRole) {
      console.error(
        "Attempted to delete a role that can't be found in the role library."
      );
      return;
    }
