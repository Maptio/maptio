...
export class CircleMenuComponent {
  ...
  /**
   * Returns a path string for the main circle arc segment of the blob (from B to C).
   */
  getMainCircleArcPath(): string {
    const bubbleRadius = 37;
    const angleD = (-105 * Math.PI) / 180;
    const angleA = (15 * Math.PI) / 180;
    const bubbleTopX = bubbleRadius * Math.cos(angleD);
    const bubbleTopY = bubbleRadius * Math.sin(angleD);
    const bubbleRightX = bubbleRadius * Math.cos(angleA);
    const bubbleRightY = bubbleRadius * Math.sin(angleA);
    ...
    // Move to B, arc to C
    return [
      `M ${bubbleRightX} ${edgeY}`,
      `A ${defaultRadius} ${defaultRadius} 0 0 ${arcSweep} ${edgeX} ${bubbleTopY}`,
    ].join(' ');
  }

  /**
   * Returns a path string for the small circle arc segment of the blob (from D to A).
   */
  getSmallCircleArcPath(): string {
    const bubbleRadius = 37;
    const angleD = (-105 * Math.PI) / 180;
    const angleA = (15 * Math.PI) / 180;
    const bubbleTopX = bubbleRadius * Math.cos(angleD);
    ...
    const bubbleRightX = bubbleRadius * Math.cos(angleA);
    const bubbleRightY = bubbleRadius * Math.sin(angleA);
    // The small circle arc from D (bubbleTopX, bubbleTopY) to A (bubbleRightX, bubbleRightY)
    // This is the last arc in getBlobPath
    return [
      `M ${bubbleTopX} ${bubbleTopY}`,
      `A ${bubbleRadius} ${bubbleRadius} 0 0 1 ${bubbleRightX} ${bubbleRightY}`,
    ].join(' ');
  }

  /**
   * Returns a path string for the bezier curve from A to B (rightmost to bottom intersection).
   */
  getBezierABPath(): string {
    const bubbleRadius = 37;
    const angleA = (15 * Math.PI) / 180;
    const bubbleRightX = bubbleRadius * Math.cos(angleA);
    const bubbleRightY = bubbleRadius * Math.sin(angleA);
  ...
  /**
   * Returns a path string for the bezier curve from D to C (topmost to left intersection).
   */
  getBezierDCPath(): string {
    const bubbleRadius = 37;
    const angleD = (-105 * Math.PI) / 180;
    const bubbleTopX = bubbleRadius * Math.cos(angleD);
    const bubbleTopY = bubbleRadius * Math.sin(angleD);
    ...
    const ctrlDCx = midDCx + (toMainDCx / lenDC) * 30;
    const ctrlDCy = midDCy + (toMainDCy / lenDC) * 30;
    // Path: Move to D, bezier to C
    return [
      `M ${bubbleTopX} ${bubbleTopY}`,
      `Q ${ctrlDCx} ${ctrlDCy} ${edgeX} ${bubbleTopY}`,
    ].join(' ');
  }

  /**
   * Returns a path string for a shadow blob that ends before the main circle.
   * Uses a slightly larger radius for the main circle arc segment.
   */
  getShadowBlobPath(): string {
    const bubbleRadius = 37;
    // Angles in radians
    const angleD = (-105 * Math.PI) / 180; // -10° (topmost)
    const angleA = (15 * Math.PI) / 180; // 100° (rightmost)
    ...
    // The main circle center in menu's local coordinates
    const defaultRadius = this.defaultRadius;
    const shadowRadius = defaultRadius + 24; // 24px larger for shadow arc, further from main circle
    const offset = 1.08 * defaultRadius * 0.707;
    const mainCircleX = -offset;
    const mainCircleY = offset;
    const rSquared = defaultRadius * defaultRadius;
    ...
    // Calculate angles for B and C relative to main circle center
    const angleB = Math.atan2(edgeY - mainCircleY, bubbleRightX - mainCircleX);
    const angleC = Math.atan2(bubbleTopY - mainCircleY, edgeX - mainCircleX);
    // Move B' and C' closer together by interpolating toward the midpoint angle
    const midAngle = (angleB + angleC) / 2;
    const angleSpread = (angleB - angleC) / 2;
    const shrinkFactor = 0.5; // 1 = full spread, 0 = both at midpoint
    const newAngleB = midAngle + angleSpread * shrinkFactor;
    const newAngleC = midAngle - angleSpread * shrinkFactor;
    // New arc endpoints for shadow arc (B' and C')
    const shadowB_X = mainCircleX + shadowRadius * Math.cos(newAngleB);
    const shadowB_Y = mainCircleY + shadowRadius * Math.sin(newAngleB);
    const shadowC_X = mainCircleX + shadowRadius * Math.cos(newAngleC);
    const shadowC_Y = mainCircleY + shadowRadius * Math.sin(newAngleC);

    // --- Bezier control points (same as main blob) ---
    const midABx = (bubbleRightX + bubbleRightX) / 2;
    const midABy = (bubbleRightY + edgeY) / 2;
    const toMainABx = mainCircleX - bubbleRightX;
    const toMainABy = mainCircleY - (bubbleRightY + edgeY) / 2;
    ...
    // Main circle arc from B' to C' (with shadowRadius)
    const vB = [shadowB_X - mainCircleX, shadowB_Y - mainCircleY];
    const vC = [shadowC_X - mainCircleX, shadowC_Y - mainCircleY];
    const cross = vB[0] * vC[1] - vB[1] * vC[0];
    const arcSweep = cross > 0 ? 1 : 0;

    // Path: A -> B (bezier), B' -> C' (main circle arc with larger radius, shorter angle), C -> D (bezier), D -> A (small circle arc)
    return [
      `M ${bubbleRightX} ${bubbleRightY}`,
      `Q ${ctrlABx} ${ctrlABy} ${shadowB_X} ${shadowB_Y}`,
      `A ${shadowRadius} ${shadowRadius} 0 0 ${arcSweep} ${shadowC_X} ${shadowC_Y}`,
      `Q ${ctrlDCx} ${ctrlDCy} ${bubbleTopX} ${bubbleTopY}`,
      `A ${bubbleRadius} ${bubbleRadius} 0 0 1 ${bubbleRightX} ${bubbleRightY}`,
      'Z',
    ].join(' ');
