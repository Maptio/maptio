...
const loadLanguageJson = (language: string) =>
  require(`../languages/${language}.json`);
...
const messageByLanguage = (
  ...
  if (!message) {
    throw new Error(
      `Message not found for key: ${messageKey} when using language: ${language}`
    );
  }
  return message;
};
...
const createLanguageCondition = (
  ...
  const middleLanguageCondition = `
    {% elsif request_language == '${language}' %}
        ${message}
    `;
...
export const localizeMessage = (messageKey: string) => {
  ...
  const firstCondition = createLanguageCondition(
    messageKey,
    firstLanguage,
    ConditionType.FIRST
  );
  ...
  const defaultCondition = createLanguageCondition(
    messageKey,
    firstLanguage,
    ConditionType.LAST
  );
