...
describe("export.service.ts", () => {
    ...
    describe("isCsvFile", () => {
        it("should return true when file is .csv", inject([FileService], (target: FileService) => {
    ...
    describe("getHeaderArray", () => {
        it("should return header for well-formed .csv", inject([FileService], (target: FileService) => {
            let csvRecordsArray = ["First name,Lastname, email", "one,Maptio, one@maptio.com"]
    ...
    describe("validateHeaders", () => {
        it("should return true when header is matching", inject([FileService], (target: FileService) => {
        ...
        it("should return false when header is in wrong order", inject([FileService], (target: FileService) => {
            let fileHeader = ["First name", "Lastname", " email"];
        ...
        it("should return false when header is of wrong length", inject([FileService], (target: FileService) => {
            let fileHeader = ["First name", "Lastname", " email"];
    ...
    describe("getDataRecordsArrayFromCSVFile", () => {
        it("should return correct array for well-formed csv ", inject([FileService], (target: FileService) => {
            let csvRecordsArray = ["First name,Lastname,email", "one,Maptio,one@maptio.com", "two,Maptio,two@maptio.com"]
            let actual = target.getDataRecordsArrayFromCSVFile(csvRecordsArray, 3, true, Constants.tokenDelimeter);
        ...
        xit("should throw when header length is incorrect and check validateHeaderAndRecordLengthFlag= true ", inject([FileService], (target: FileService) => {
            ...
            }).toThrow("Record at line number 0 contain 3 tokens, and is not matching with header length of :2");
        }));
    });
