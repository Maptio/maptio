...
describe("integrations.component.ts", () => {
    ...
    describe("revoke token", () => {
        it("calls the right dependencies", async(() => {
            ...
            expect(spyTeamFactory).toHaveBeenCalledWith(jasmine.objectContaining({
                team_id: "id",
                name: "Team",
    ...
    describe("update Team", () => {
        it("calls the right dependencies", () => {
            ...
            spyOn(target.debugElement.injector.get(TeamFactory), "upsert").and.returnValue(Promise.resolve(true))

            component.updateTeam("TOKEN", {
                url: "app_url",
                channel: "channel",
            ...
            expect(target.debugElement.injector.get(TeamFactory).upsert).toHaveBeenCalledWith(jasmine.objectContaining({
                team_id: "id",
                name: "Team",
                slack: new SlackIntegration({
                    access_token: "TOKEN",
                    ...
                    team_id : "workspaceId",
                    incoming_webhook: {
                        url: "app_url", channel: "channel", channel_id: "channelId", configuration_url: "configuration_url"
                    }
                }),
                settings: { authority: "A", helper: "H" },
                members: []
